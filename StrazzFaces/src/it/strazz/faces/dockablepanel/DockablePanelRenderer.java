...
public class DockablePanelRenderer extends CoreRenderer {
	...
	public void decode(FacesContext context, UIComponent component) {
		...
		dockablepanelComponent.setSubmittedValue(Boolean
	...
	public void encodeBegin(FacesContext context, UIComponent component)
		...
		if (dockablepanelComponent.getPosition() == null
				|| dockablepanelComponent.getPosition().isEmpty())
			dockablepanelComponent.setPosition(DockablePanel.DEFAULT_POSITION);
		else
			dockablepanelComponent.setPosition(dockablepanelComponent
					.getPosition().toLowerCase());

		if ("bottom".equalsIgnoreCase(dockablepanelComponent.getPosition()))
			encodeBeginBottom(dockablepanelComponent, writer);
		else
			encodeBeginTop(dockablepanelComponent, writer); // Same for Left,
															// Right, Top
	}
	...
	private void encodeScript(FacesContext context,
			DockablePanel dockablepanelComponent) throws IOException {
	...
	private String getExtPanelPosition(DockablePanel dockablepanelComponent) {
		...
		return DockablePanel.AVAIABLE_POSITIONS.contains(position) ? position
				: DockablePanel.DEFAULT_POSITION;
	}
	...
	private void encodeMarkup(FacesContext context,
		...
		if ("bottom".equalsIgnoreCase(dockablepanelComponent.getPosition()))
			...
			encodeMarkupTop(dockablepanelComponent, writer); // Same for Left,
																// Right, Top
	}

	private void encodeBeginTop(DockablePanel dockablepanelComponent,
		...
		writer.writeAttribute(
				"class",
				"dockablepanel-content ui-panel ui-widget-content ui-corner-all ui-shadow",
				null);
		writer.writeAttribute("style", "display:none", null);
	}

	private void encodeBeginBottom(DockablePanel dockablepanelComponent,
			ResponseWriter writer) throws IOException {
		// ExtPanel
		writer.startElement("div", dockablepanelComponent);
		writer.writeAttribute("id", dockablepanelComponent.getClientId(), null);
		...
		writer.writeAttribute(
				"class",
				"dockablepanel-header ui-panel ui-widget-content ui-corner-all ui-shadow",
				null);

		// ExtPanel Header Content
		writer.startElement("div", dockablepanelComponent);
		writer.writeAttribute("class",
				"dockablepanel-header-content ui-widget-header ui-corner-all",
				null);

		// Icon
		writer.startElement("span", dockablepanelComponent);
		writer.writeAttribute("class", "ui-icon ui-icon-circle-triangle-n",
				null);
	...
	private void encodeMarkupTop(DockablePanel dockablepanelComponent,
			ResponseWriter writer) throws IOException {
		...
		// Icon
		writer.startElement("span", dockablepanelComponent);
		writer.writeAttribute("class", "ui-icon ui-icon-circle-triangle-s",
				null);
		writer.endElement("span");
